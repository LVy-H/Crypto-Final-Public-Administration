 ...
 */
export async function generateKeyPair(algorithm = 'ML-DSA-65') {
    let sig;
    switch (algorithm) {
        case 'ML-DSA-44':
 ...
 */
export async function signData(secretKey, message, algorithm = 'ML-DSA-65') {
    let sig;
    switch (algorithm) {
        case 'ML-DSA-44':
            sig = await createMLDSA44();
 ...
 */
export async function verifySignature(publicKey, message, signature, algorithm = 'ML-DSA-65') {
    let sig;
    switch (algorithm) {
        case 'ML-DSA-44':
            sig = await createMLDSA44();
 ...
 */
export async function generateCsrData(subjectDn, keyPair) {
    ...
    let offset = 0;
    view.setUint32(offset, subjectBytes.length, true);
    offset += 4;
    pkg.set(subjectBytes, offset);
    offset += subjectBytes.length;
    view.setUint32(offset, keyPair.publicKey.length, true);
    offset += 4;
    pkg.set(keyPair.publicKey, offset);
    offset += keyPair.publicKey.length;
    view.setUint32(offset, signature.length, true);
    offset += 4;
 ...
 */
export async function storePrivateKey(secretKey, userId, passphrase) {
 ...
 */
export async function retrievePrivateKey(userId, passphrase) {
    const db = await openKeyStore();
    return new Promise((resolve, reject) => {
