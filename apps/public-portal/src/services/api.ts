...
export interface User {
    username: string;
    role: 'USER' | 'ADMIN' | 'OFFICER' | 'CA_OPERATOR';
...
export interface TsaInfo {
    ...
    acceptedDigestAlgorithms: string[];
...
class ApiService {
    ...
    async register(username: string, password: string, role: string = 'USER'): Promise<ApiResponse<void>> {
        try {
    ...
    async approveKyc(username: string): Promise<ApiResponse<void>> {
        try {
    ...
    async uploadDocument(file: File): Promise<ApiResponse<{ docId: string }>> {
        try {
    ...
    async getDocumentHash(docId: string): Promise<ApiResponse<{ hash: string }>> {
        try {
    ...
    async verifyAsic(asicFile: File | Blob): Promise<ApiResponse<VerificationResult>> {
        try {
            const formData = new FormData();
            formData.append('file', asicFile, 'document.asic');
    ...
    async getCaCertificate(): Promise<ApiResponse<Blob>> {
        try {
    ...
    async requestCertificate(csrBase64: string): Promise<ApiResponse<Blob>> {
        try {
    ...
    async getTsaInfo(): Promise<ApiResponse<TsaInfo>> {
        try {
    ...
    async requestTimestamp(hash: Uint8Array): Promise<ApiResponse<Blob>> {
        try {
            const body = hash.buffer.slice(hash.byteOffset, hash.byteOffset + hash.byteLength) as ArrayBuffer;
