// Proxy GET /api/v1/user/stats to api-gateway
...
export default defineEventHandler(async (event) => {
    ...
    try {
        const response = await $fetch(`${upstreamUrl}/api/v1/user/stats`, {
            method: 'GET',
            headers: {
        ...
        console.error('User stats error:', error.data || error.message)
        throw createError({
            statusCode: error.statusCode || 500,
            message: error.data?.message || 'Failed to fetch user stats'
        })
    }
})
